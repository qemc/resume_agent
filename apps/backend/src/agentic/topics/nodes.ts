...
export async function unifyText(state: typeof State.State) {
    ...
    try {
        ...
        if (result.topics.length !== state.careerPathTopics.length) {
            return nodeError(
                new Error(`Unify node error: returned topics list length is not equal to the initial topics list length. Initial topics list length: ${state.careerPathTopics.length}, Length of list returned by LLM: ${result.topics.length}`)
            );
        }
